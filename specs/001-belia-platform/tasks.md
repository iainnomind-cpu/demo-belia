...
- [ ] T009 [US1] Crear hook `useCategories` en `src/hooks/useCategories.ts` para obtener árbol de navegación.
- [ ] T010 [US1] Implementar componente `Header` con buscador y mega-menú (Desktop) en `src/components/layout/Header.tsx`
- [ ] T011 [US1] Implementar Menú Hamburguesa (Mobile) con Framer Motion en `src/components/layout/MobileMenu.tsx`
- [ ] T012 [US1] Implementar layout de Home en `src/pages/storefront/HomePage.tsx`
- [ ] T013 [US1] Implementar vista de listado de categoría vacía (UI) en `src/pages/storefront/CategoryPage.tsx`
- [ ] T014 [US1] Validar explícitamente la UX móvil en vista previa.
...
- [ ] T016 [US2] Crear hook `useProducts` en `src/hooks/useProducts.ts` implementando lógica de scroll infinito y filtros.
- [ ] T017 [US2] Implementar componente `ProductCard` (solo precio público/promo) en `src/components/catalog/ProductCard.tsx`
- [ ] T018 [US2] Implementar barra lateral de filtros en `src/components/catalog/FiltersSidebar.tsx`
- [ ] T019 [US2] Actualizar `CategoryPage.tsx` para integrar `FiltersSidebar` y listado de productos con scroll infinito.
- [ ] T020 [US2] Implementar `ProductDetailPage.tsx` (Ficha de Producto) respetando 3-clicks rule.
...
- [ ] T027 [US4] Implementar `AdminRoute` guard en `src/components/auth/AdminRoute.tsx`
- [ ] T028 [US4] Crear vista de listado y detalle de Órdenes en `src/pages/admin/OrdersAdminPage.tsx` (con lógica progresiva de estados).
...
- [ ] T032 [US5] Crear Supabase Edge Function `sync-catalog` en `supabase/functions/sync-catalog/index.ts` integrando API v4 de Sheets.
- [ ] T033 [US5] Implementar UI de Sincronización (Botón + Modal de Diff Preview) en `src/pages/admin/SyncAdminPage.tsx`.
- [ ] T034 [US5] Implementar lógica de Soft Delete y protección 'manual' en la función de sync.
...
- [ ] T036 [US6] Crear formulario público en `src/pages/storefront/SupplierFormPage.tsx` y guardar en `suppliers` con estado 'pendiente'.
- [ ] T037 [US6] Crear Supabase Edge Function `approve-supplier` en `supabase/functions/approve-supplier/index.ts` con SMTP de Gmail.
- [ ] T038 [US6] Crear gestor de Solicitudes en `src/pages/admin/SuppliersAdminPage.tsx` (aprobar/rechazar llamando a Edge Function).
- [ ] T039 [US6] Modificar `ProductCard` y Ficha de Producto para utilizar `get_supplier_products()` si el usuario tiene rol `proveedor`.
